...
%%% (Fig. 4, mutations in the YFP strains)   %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fprintf('Printing Figure 4 opgG mlaA\n');
f = filesep;
...
if ~exist([anaFolder_opgG f imgsOpgGfn],'file')
    ...
    % collect all images into a matrix
...
letter_fs = 14;
imgNmsVecWT = {'WmWy'};
imgNmsVecY = {'WmGy','WmJy','WmJGy'}; 
imgTitsWT = {'WT','WT'};
imgTitsY = {{'WT','opgG*'},{'WT','waaJ*'},{'WT','waaJ*opgG*'}}; 
margin_x_left = 80;
margin_x_right = 20;
plotPhg = [4,11,12];
...
[~,relImgsY] = ismember(imgNmsVecY,imgNms);
w = 1.6; 
h = 1.6; 
gap_x = 0.2; 
gap_y = 1.5;
margin_x = 1.5;
margin_y = 0.5;
gap_mlaAopgG = (h+gap_y)*numel(imgNmsVecY);
...
for phg = 1:numel(plotPhg)
    ...
    ax.XTick = [];
    ax.YTick = [];
    tit1 = ['\color[rgb]{' sprintf('%1.2f,%1.2f,%1.2f', titClr(1,:)) '}' imgTitsWT{1}];
    tit2 = ['\color[rgb]{' sprintf('%1.2f,%1.2f,%1.2f', titClr(2,:)) '}'  imgTitsWT{2}];
...
% Plot Ys
for phg = 1:numel(plotPhg)
    for bac = 1:numel(relImgsY)
        alldrops = imgs(drops_centers(1,2)-r:drops_centers(8,2)+r,drops_centers(1,1)-r:drops_centers(96,1)+r,:,relImgsY(bac));
        maxR = double(median(maxk(reshape(alldrops(:,:,1),[],1),maxMinPxl)))/255;
        maxY = double(median(maxk(reshape(alldrops(:,:,2),[],1),maxMinPxl)))/255;
        minR = double(median(mink(reshape(alldrops(:,:,1),[],1),maxMinPxl)))/255;
        minY = double(median(mink(reshape(alldrops(:,:,2),[],1),maxMinPxl)))/255;
        crpImg = imgs(drops_centers((plotPhg(phg)-1)*8+dilutionVec(plotPhg(phg)),2)-r:...
            drops_centers((plotPhg(phg)-1)*8+dilutionVec(plotPhg(phg)),2)+r,...
            drops_centers((plotPhg(phg)-1)*8+1,1)-r:drops_centers((plotPhg(phg)-1)*8+1,1)+r,:,relImgsY(bac));
        crpAdjImg = imadjust(crpImg,[minR,minY,minR; maxR,maxY,maxR]);
        ...
        ax.XTick = [];
        ax.YTick = [];
        tit1 = ['\color[rgb]{' sprintf('%1.2f,%1.2f,%1.2f', titClr(1,:)) '}' imgTitsY{bac}{1}];
        tit2 = ['\it\color[rgb]{' sprintf('%1.2f,%1.2f,%1.2f', titClr(2,:)) '}'  imgTitsY{bac}{2}];
        t = title({tit1,tit2},'fontsize',10, 'Interpreter', 'tex');
    end 
end
%% MlaA
...
for phg = 1:numel(plotPhg)
    ...
    ax.XTick = [];
    ax.YTick = [];
    tit1 = ['\it\color[rgb]{' sprintf('%1.2f,%1.2f,%1.2f', titClr(1,:)) '}' imgTitsWT{1}];
    tit2 = ['\color[rgb]{' sprintf('%1.2f,%1.2f,%1.2f', titClr(2,:)) '}'  imgTitsWT{2}];
    t = title({tit1,tit2},'fontsize',10, 'Interpreter', 'tex');
    ylh = ylabel(phgTitles{phg});
    ylp = get(ylh, 'Position');
...
for phg = 1:numel(plotPhg)
    for bac = 1:numel(relImgsY)
        ...
        ax.XTick = [];
        ax.YTick = [];
        tit1 = ['\it\color[rgb]{' sprintf('%1.2f,%1.2f,%1.2f', titClr(1,:)) '}' imgTitsY{1}];
        tit2 = ['\color[rgb]{' sprintf('%1.2f,%1.2f,%1.2f', titClr(2,:)) '}'  imgTitsY{2}];
        t = title({tit1,tit2},...
...
print([figure_location f 'Figure4'],'-depsc2','-r300')
print([figure_location f 'Figure4.png'],'-dpng','-r300')
